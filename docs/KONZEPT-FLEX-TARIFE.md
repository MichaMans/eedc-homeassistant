...
| **P-6** „eine Kaskade gehört einem Zähler" | `monats_fakten.py:2472-2480` wendet den Hauszähler-Flex-Ø bewusst auf den Wallbox-Tarif an (Konsistenz im `TarifFakten`) | Regel und Code sagen Verschiedenes; ohne zählerscharfe Tarifzuordnung ist der Code vertretbar | **SOLL-Satz präzisieren** („gilt erst mit eigenem Zähler"), Code lassen |
...
| **P-6** „eine Kaskade gehört einem Zähler" | **SOLL-Satz präzisiert** | gilt erst, wenn eedc einen eigenen Zähler je Komponente modelliert; bis dahin wendet `monats_fakten.py` den Hauszähler-Flex-Ø bewusst auch auf den Wallbox-Tarif an (Konsistenz im `TarifFakten`) |
