...
| ~~12~~ | ~~**Break-Even-Kurve als Kalender-Treppe** (N-525, Radiocarbonat T89667 #342)~~ | ~~mittel~~ | ✅ **gebaut 2026-09-18.** Jede ROI-Zeile zählt Mehrkosten und Netto-Jahres-Einsparung ab ihrem Anschaffungsjahr, sonstige Positionen stufen im Jahr ihrer Buchung; das Break-Even-Jahr ist der Schnittpunkt dieser Reihe (F-19: Kachel und Kurve nennen dasselbe Jahr). Layer-SoT `kapitalrechnung.amortisations_verlauf`, Route `get_roi_dashboard` liefert `amortisations_verlauf`, der Client zeichnet nur (ADR-001). **Die Dauer bleibt Modell A** — die Treppe ändert keine Annahme über die Zukunft, sie verteilt nur auf die Zeitachse; ⚠ nicht mit dem in §5/§7 verworfenen „Modell B" verwechseln. Benannte Grenze: ein PV-System hat eine Einsparung, seine Kosten stufen je Komponente. Proben `test_amortisations_verlauf.py`, `test_roi_amortisation_jahr.py`, Konformitätsprobe `test_erfuellt_schritt12_*`. |
