...
| **S1b — WP-Strom voll belastet** | ADR-002/**P9**, zweiter Fall; `alternativkosten.py`, `aussichten/finanzen.py`, `wp_wirtschaftlichkeit.py`, `calculations.py` | `test_n459_wp_strom_voll_belastet.py` (⚠ **die Datei sagt es selbst:** Regressionen, kein Wächter — nach dem Bau existiert nur noch **eine** Bildungsvorschrift, es kann nichts mehr driften) | Regression |
